import React, { useEffect, useMemo, useState } from 'react';
import { eventosAPI } from '../api/eventosAPI';
import { useAuth } from '../contexts/AuthContext';
import './AdminShared.css';
import './GestionCiudades.css';

// Utilidad para extraer data del axios response
const unwrap = (resp) => (resp && resp.data !== undefined ? resp.data : resp) || [];

export default function GestionCiudades() {
  const { isAdmin } = useAuth() || { isAdmin: false };
  const [ciudades, setCiudades] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [busqueda, setBusqueda] = useState('');
  const [form, setForm] = useState({ id: null, nombre: '' });
  const [saving, setSaving] = useState(false);
  const [deletingId, setDeletingId] = useState(null);

  const cargarCiudades = async () => {
    setLoading(true);
    setError(null);
    try {
      const resp = await eventosAPI.getCiudades();
      const data = unwrap(resp);
      setCiudades(Array.isArray(data) ? data : data?.content || []);
    } catch (e) {
      console.error('Error al cargar ciudades', e);
      setError(e?.message || 'Error al cargar ciudades');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    cargarCiudades();
  }, []);

  const limpiarFormulario = () => setForm({ id: null, nombre: '' });

  const onChange = (e) => {
    const { name, value } = e.target;
    setForm((f) => ({ ...f, [name]: value }));
  };

  const crearActualizar = async (e) => {
    e.preventDefault();
    if (!form.nombre?.trim()) {
      setError('El nombre de la ciudad es obligatorio');
      return;
    }
    setSaving(true);
    setError(null);
    try {
      if (form.id) {
        await eventosAPI.updateCiudad(form.id, { nombre: form.nombre.trim() });
      } else {
        await eventosAPI.createCiudad({ nombre: form.nombre.trim() });
      }
      await cargarCiudades();
      limpiarFormulario();
    } catch (e2) {
      console.error('Error al guardar ciudad', e2);
      setError(e2?.message || 'No se pudo guardar la ciudad');
    } finally {
      setSaving(false);
    }
  };

  const editar = (c) => {
    setForm({ id: c.id ?? c._id ?? c.codigo ?? null, nombre: c.nombre ?? '' });
  };

  const eliminar = async (c) => {
    const id = c.id ?? c._id ?? c.codigo;
    if (!id) return;
    if (!confirm(`¿Eliminar la ciudad "${c.nombre ?? id}"?`)) return;
    setDeletingId(id);
    setError(null);
    try {
      await eventosAPI.deleteCiudad(id);
      await cargarCiudades();
    } catch (e3) {
      console.error('Error al eliminar ciudad', e3);
      setError(e3?.message || 'No se pudo eliminar la ciudad');
    } finally {
      setDeletingId(null);
    }
  };

  const filtradas = useMemo(() => {
    const q = busqueda.trim().toLowerCase();
    if (!q) return ciudades;
    return ciudades.filter((c) =>
      [c.nombre, c.pais, c.provincia, c.departamento, c.id, c._id]
        .filter(Boolean)
        .some((v) => String(v).toLowerCase().includes(q))
    );
  }, [ciudades, busqueda]);

  if (!isAdmin) {
    return (
      <div className="p-6">
        <div className="max-w-3xl mx-auto bg-white/5 border border-white/10 rounded-xl p-6">
          <h2 className="text-white text-lg font-semibold">No autorizado</h2>
          <p className="text-white/60 text-sm">Esta sección es exclusiva para administradores.</p>
        </div>
      </div>
    );
  }

  return (
    <div className="admin-page p-6 gestion-ciudades">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">
              Gestión de Ciudades
            </h1>
            <p className="text-white/60 text-sm mt-1">
              Crea, edita y elimina ciudades para asociar a los eventos.
            </p>
          </div>
        </div>

        {error && (
          <div className="admin-alert">
            {error}
          </div>
        )}

        {/* Formulario */}
        <form onSubmit={crearActualizar} className="admin-card">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm text-white/70 mb-1">Nombre</label>
              <input
                type="text"
                name="nombre"
                value={form.nombre}
                onChange={onChange}
                placeholder="Ej. Quito"
                className="admin-input w-full"
              />
            </div>

            <div className="flex items-end space-x-2">
              <button
                type="submit"
                disabled={saving}
                className="admin-btn admin-btn-primary"
              >
                {form.id ? (saving ? 'Actualizando...' : 'Actualizar') : (saving ? 'Creando...' : 'Crear')}
              </button>
              {form.id && (
                <button
                  type="button"
                  onClick={limpiarFormulario}
                  className="admin-btn"
                >
                  Cancelar
                </button>
              )}
            </div>
          </div>
        </form>

        {/* Barra de acciones */}
        <div className="admin-toolbar flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="relative">
            <input
              type="text"
              placeholder="Buscar ciudad..."
              value={busqueda}
              onChange={(e) => setBusqueda(e.target.value)}
              className="admin-input w-full md:w-80 pl-10"
            />
            <svg className="w-5 h-5 text-white/40 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </div>
          <button
            onClick={cargarCiudades}
            disabled={loading}
            className="admin-btn"
          >
            {loading ? 'Actualizando...' : 'Refrescar'}
          </button>
        </div>

        {/* Tabla */}
        <div className="admin-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="admin-table min-w-full">
              <thead className="bg-white/5">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">ID</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-white/60 uppercase tracking-wider">Nombre</th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-white/60 uppercase tracking-wider">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/10">
                {filtradas.map((c) => {
                  const id = c.id ?? c._id ?? c.codigo ?? '-';
                  return (
                    <tr key={id} className="hover:bg-white/5">
                      <td className="px-4 py-3 text-white/80 text-sm">{id}</td>
                      <td className="px-4 py-3 text-white text-sm font-medium">{c.nombre || '-'}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => editar(c)}
                            className="px-3 py-1.5 rounded-lg text-white/90 bg-white/10 hover:bg-white/20 text-sm"
                          >
                            Editar
                          </button>
                          <button
                            onClick={() => eliminar(c)}
                            disabled={deletingId === id}
                            className="px-3 py-1.5 rounded-lg text-red-200/90 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-sm disabled:opacity-60"
                          >
                            {deletingId === id ? 'Eliminando...' : 'Eliminar'}
                          </button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
                {(!loading && filtradas.length === 0) && (
                  <tr>
                    <td colSpan={3} className="px-4 py-8 text-center text-white/50 text-sm">
                      No hay ciudades registradas.
                    </td>
                  </tr>
                )}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  );
}
